...
% EVENT_SELECTOR - Selects NBACK events
% The 2-back task starts with instructions. Participants have to press a
% button to start the task. Hence the instructions have a variable
% duration, yet the task is always 3 minutes in duration. 
% We select the end of the task (beginning of the pvt instructions) as 
% the event, then select the preceding 3 minutes of eeg data.
   
    properties
        StimType   = 'PVT+';   % Type of the stimulus onset event
        RespType   = 'TRSP';  % Type of the response event
        TimingType = 'AUD';    % Type of the event timing event associated to a StimType event
        StimCel    = []; 
        RespValue  = [];        
        Negated    = false;
