...
Eventually we want to extract features separately for each experimental
sub-block. Thus, it makes sense to split our original data files into 12
`baseline` sub-blocks, 12 `NBACK` sub-blocks, 12 `PVT` sub-blocks, 12 `SAC` sub-blocks, 
12 `SUBJ` sub-blocks, 12 `RS-EO` sub-blocks and 12 `RS-EC` sub-blocks. That is, each raw 
data file is split into 72 files.
...
The onset and/or end of sub-block in the BATMAN dataset can be identified by an event. At 
the beginning of each block instructions about the task were given. Since the instructions 
were of variable duration across subjects and tasks, the end event for each task was used 
to select the data.
...
1. Select the relevant sub-block event. This step should produce 12 such events for each 
   of the seven individual sub-block events.

2. For each event selected in 1., let's assume that the time of the event is `t`, define 
   the offset and duration.
   Then, produce seven splits:

Event  | Split name                     | Offset from `t` (mins) | Duration (mins)
-------|--------------------------------| ---------------------- | ------------------
`TRAN` | `[dataName]_baseline_[block#]` | 0                      | 15.5
`pvt+` | `[dataName]_nback_[block#]`    | -3                     | 3
`sac+` | `[dataName]_pvt_[block#]`      | -3                     | 3
`    ` | `[dataName]_sac_[block#]`      | -1                     | 1
`    ` | `[dataName]_subj_[block#]`     |
`eoeo` | `[dataName]_rs-eo_[block#]`    | 0                      | 3
`ecec` | `[dataName]_rs-ec_[block#]`    | 0                      | 3


#### Splitting strategy implementation

One detail that is still missing is how to ensure that the produced splits are
given meanigful names. E.g. we would like the _RS-EO_ split for the second
experimental manipulation block of file `btmn_0001_eeg_afternoon.mff` to be called
`btmn_0001_eeg_afternoon_rs-eo_2`. Node _split_ names the generated splits according to
...
myEvSel = batman.baseline_selector;
...
myNode = split.new(...
    ...
    'Offset',            0, ...             % Must be in seconds
    'Duration',          15.5*60, ...       % Also in seconds
